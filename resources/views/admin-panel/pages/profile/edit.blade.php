...
<main class="form-signing w-500 m-auto">
    <form class="form" action="{{ route('update.profile', ['id' => auth()->user()->id]) }}" method="POST">
        @csrf
        @method('PUT')
        <div class="fs-2 fw-bold text-center mb-4">ویرایش پروفایل</div>
        ...
        <div class="mb-3">
            ...
            <input type="text" name="name" class="form-control shadow" value="{{ auth()->user()->name }}" />
            <div class="text-form text-danger">@error('name'){{ $message }}@enderror</div>
        </div>

        <div class="mb-3">
            <label class="form-label">ایمیل</label>
            <input type="email" name="email" class="form-control shadow" value="{{ auth()->user()->email }}" />
